...
class EigenbotJointPub():
	...
	def main_loop(self):
		if simplified_mode == 1:
			...
			print("Complex Model")
			omega = 250/(np.pi*2) #oscillation frequency in rad/s
			gamma = 40 #from paper
			gait_a = np.pi/18 #from paper
			gait_b = np.pi/6 #from paper
			gait_n = 4 #from paper
			...
			while not rospy.is_shutdown():    
				...
				for i in range(self.num_joints):
					...
					if simleg_i == 0:
						CPG_index = 1
						if joint_i == 0: #proximal joint
							joint_state.position[i] = (cpg_x_next[CPG_index]) + self.initial_joint_positions[i]
						if joint_i == 1: #intermediate joint
							joint_state.position[i] = (cpg_y_next[CPG_index]) + self.initial_joint_positions[i]
					if simleg_i == 1:
						CPG_index = 3
						if joint_i == 0: #proximal joint
							joint_state.position[i] = (cpg_x_next[CPG_index]) + self.initial_joint_positions[i]
